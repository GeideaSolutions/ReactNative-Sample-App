...
class HomeScreen extends Component {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      const currency = await EncryptedStorage.getItem("currency");
      const showEmailString = await EncryptedStorage.getItem("showEmail");
      const billingAddressString = await EncryptedStorage.getItem(
        "billingAddress"
      );
      const shippingAddressString = await EncryptedStorage.getItem(
        "shippingAddress"
      );
      ...
      const customerEmail = await EncryptedStorage.getItem("customerEmail");
      const phoneNumber = await EncryptedStorage.getItem("phoneNumber");
      const headerColor = await EncryptedStorage.getItem("headerColor");
      ...
      const merchantReferenceID = await EncryptedStorage.getItem(
        "merchantReferenceID"
      );
      const lang = await EncryptedStorage.getItem("lang");
      const callbackUrl = await EncryptedStorage.getItem("callbackUrl");
      const returnUrl = await EncryptedStorage.getItem("returnUrl");
      const code = await EncryptedStorage.getItem("code");

      const selectedEnvironment = await EncryptedStorage.getItem(
        "selectedEnvironment"
      );
      let selectedEnvironmentIndex;
      try {
        const stringValue = await EncryptedStorage.getItem(
      ...
      const initiatedBy = await EncryptedStorage.getItem("initiatedBy");
  ...
  onSave = async () => {
    try {
      ...
      await EncryptedStorage.setItem(
        "selectedEnvironmentIndex",
        this.state.selectedEnvironmentIndex.toString()
      );
      await EncryptedStorage.setItem("currency", this.state.currency);
      await EncryptedStorage.setItem("showEmail", showEmailString);
      await EncryptedStorage.setItem(
        "billingAddress",
      ...
      await EncryptedStorage.setItem("showBilling", showBillingString);
      await EncryptedStorage.setItem("hideLogo", hideLogoString);
      await EncryptedStorage.setItem("showReceipt", showReceiptString);
      await EncryptedStorage.setItem("customerEmail", this.state.customerEmail);
      await EncryptedStorage.setItem("phoneNumber", this.state.phoneNumber);
      await EncryptedStorage.setItem("headerColor", this.state.headerColor);
      await EncryptedStorage.setItem("showPhone", showPhoneString);
      await EncryptedStorage.setItem(
        "merchantReferenceID",
        this.state.merchantReferenceID
      );
      await EncryptedStorage.setItem("lang", this.state.lang);
      await EncryptedStorage.setItem("sameAddress", sameAddressString);
      await EncryptedStorage.setItem("callbackUrl", this.state.callbackUrl);
      await EncryptedStorage.setItem("returnUrl", this.state.returnUrl);
      await EncryptedStorage.setItem("code", this.state.code);
      await EncryptedStorage.setItem("initiatedBy", this.state.initiatedBy);
      await EncryptedStorage.setItem(
        "merchantReferenceID",
        this.state.merchantReferenceID
      );
  ...
  renderDropDown(varName, options) {
    ...
    if (varName === "initiatedBy") {
      placeholderText =
        this.state.initiatedBy === "" ? "Initiated By" : this.state.initiatedBy;
    } else {
  ...
  render() {
    const { publicKey, apiPassword, isProdSelected, isTestSelected, token } =
      this.state;
        ...
        <ScrollView style={{ margin: 20 }}>
          <View style={styles.pickercontainer}>
            ...
            <SelectDropdown
              ...
              defaultButtonText={
                environments[this.getSelectedEnvironmentIndex()]
              }
